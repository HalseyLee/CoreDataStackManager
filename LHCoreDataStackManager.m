...
//  手动创建CoreDataStack
//
//  Created by Halsey on 5/3/16.
//  Copyright © 2016 Halsey. All rights reserved.
//

#import "LHCoreDataStackManager.h"

@implementation LHCoreDataStackManager

+ (LHCoreDataStackManager *)sharedInstance{
    
    static LHCoreDataStackManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[LHCoreDataStackManager alloc] init];
    });
    return manager;
}
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator{
    ...
    //拼接url路径
    NSURL *url = [[self getDocumentUrlPath] URLByAppendingPathComponent:[NSString stringWithFormat:@"%@.db",kFileName] isDirectory:YES];
